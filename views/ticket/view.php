                    ...
                    if($model->messages[$i]['author'] === 'member'){
                        if($model->messages[$i]['type'] === 'text'){
                            ...
                            if(array_key_exists(5, $path) && array_key_exists(6, $path)){
                                $file = explode('.', $path[6]);
                                echo '<div class="alert alert-primary">' . $i . '.' . ' Файл от клиента: ' . yii\helpers\Html::a('Скачать', ['downloadtg/' . $bot . '-' . $path[5] . '-' . $file[0] . '-' . $file[1]], ['id' => 'tgdownload', 'class' => 'btn btn-danger btn-sm my-1', 'title' => 'Скачать', 'target' => '_self']) . '</div>';
                            }
                        }
                    }
                    else{
                        ...
                        if($model->messages[$i]['type'] === 'text'){
                            ...
                            if(array_key_exists(5, $path) && array_key_exists(6, $path)){
                                $file = explode('.', $path[6]);
                                echo '<div class="alert alert-secondary">' . $i . '.' . ' Файл от ' . $snm . ' : ' . yii\helpers\Html::a('Скачать', ['downloadtg/' . $bot . '-' . $path[5] . '-' . $file[0] . '-' . $file[1]], ['id' => 'tgdownload', 'class' => 'btn btn-danger btn-sm my-1', 'title' => 'Скачать', 'target' => '_self']) . '</div>';
                            }
                        }
                    }
                }
